...
func (w *fileWriter) AddSize(size int64) error {
	if w.closed {
		return errors.New("zip: add size to closed file")
	}
	w.rawCount.AddSize(size)
	w.compCount.AddSize(size)
	w.zipw.(*countWriter).AddSize(size)
	return nil
}
...
func (w *countWriter) AddSize(size int64) {
	w.count += size
}
